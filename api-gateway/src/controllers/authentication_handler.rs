...
pub async fn sign_in(Form(login) :Form<Login>) -> Result<impl IntoResponse, impl IntoResponse> {
    ...
    match client.get(format!("http://authentication-service:9090/sign-in/{}/{}",login.username,login.password)).send().await {
        Ok(response) => {
...
pub async fn sign_up(Form(register) :Form<Register>) -> impl IntoResponse{
    let client = Client::new() ;
    match client.get(format!("http://authentication-service:9090/sign-up/{}/{}/{}/{}/{}",
